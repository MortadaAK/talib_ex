...
ex_mama(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
      ...
      if (input0 != NULL)
  {
    ...
    return enif_make_tuple2(env, atoms->atom_error, enif_make_string(env, "fast_limit should be an double", ERL_NIF_LATIN1));
  }
  enif_get_double(env, argv[1], &input1);
  ...
  if (input0 != NULL)
  {
    ...
    return enif_make_tuple2(env, atoms->atom_error, enif_make_string(env, "slow_limit should be an double", ERL_NIF_LATIN1));
  }
