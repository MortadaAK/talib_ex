...
ex_bbands(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
  ...
  if (input0 != NULL)
  {
    ...
    return enif_make_tuple2(env, atoms->atom_error, enif_make_string(env, "nb_dev_up should be an double", ERL_NIF_LATIN1));
  }
  ...
  if (input0 != NULL)
  {
    ...
    return enif_make_tuple2(env, atoms->atom_error, enif_make_string(env, "nb_dev_dn should be an double", ERL_NIF_LATIN1));
  }
